...
export function BasicsSection({
  ...
  const handleImageSelect = async () => {
    ...
    try {
      const result = await window.api.invoke("image:select-and-optimize");

      if (result.success && result.dataUrl) {
        onUpdate("image", result.dataUrl);
          ...
          <div className="flex gap-2">
            <div className="flex-1">
              <Field
                ...
                placeholder="https://... ou utilisez le bouton pour uploader"
              />
            </div>
            <Button
              type="button"
          ...
          {basics?.image && (
            <div className="flex items-center gap-3 p-3 bg-muted/30 rounded-lg">
              ...
              <div className="flex-1 text-xs text-muted-foreground">
                {imageSizeInfo && (
                  <div>
                    Taille originale: {formatBytes(imageSizeInfo.original)} →
                    Optimisée: {formatBytes(imageSizeInfo.optimized)}
                    <span className="text-green-600 ml-2">
                      (
                      {Math.round(
                        (1 - imageSizeInfo.optimized / imageSizeInfo.original) *
                          100,
                      )}
                      % de réduction)
                    </span>
                ...
                {!imageSizeInfo && basics.image.startsWith("http") && (
                  <div>Image depuis URL (non optimisée)</div>
                )}
                {!imageSizeInfo && basics.image.startsWith("data:") && (
                  <div>Image optimisée</div>
                )}
              </div>
            </div>
          )}
        ...
        <div className="pt-4 border-t">
          ...
          <div className="grid grid-cols-3 gap-4">
            <Field
              ...
              onChange={(v) => onUpdateLocation("city", v)}
            />
            <Field
              label="Région"
              value={basics?.location?.region}
              onChange={(v) => onUpdateLocation("region", v)}
            />
            <Field
              label="Code Pays"
              value={basics?.location?.countryCode}
              onChange={(v) => onUpdateLocation("countryCode", v)}
            />
          </div>
        </div>
        ...
        <div className="pt-4 border-t">
          ...
          <div className="space-y-3">
            {basics?.profiles?.map((profile, i) => (
              <div
                key={i}
                className="flex items-end gap-3 p-3 bg-muted/30 rounded-lg relative group"
              >
                <div className="flex-1 grid grid-cols-3 gap-3">
                  <Field
                    label="Réseau"
                    value={profile.network}
                    onChange={(v) => onUpdateProfile(i, "network", v)}
                    placeholder="ex: LinkedIn"
                  />
                  <Field
                    label="Nom d'utilisateur"
                    value={profile.username}
                    onChange={(v) => onUpdateProfile(i, "username", v)}
                  />
                  <Field
                    label="URL"
                    value={profile.url}
                    onChange={(v) => onUpdateProfile(i, "url", v)}
                  />
                </div>
                <Button
                  variant="ghost"
